...
class GuestbookFormatter < FormatterBase
  ...
  def start_backup(dialogs)
    ...
    FileUtils.cp(File.expand_path('../guestbook-data/telegram-history-dump.css', __FILE__), output_dir)
    FileUtils.cp_r(File.expand_path('../guestbook-data/resources', __FILE__), output_dir)
    FileUtils.mkdir_p(output_dir + '/media')

    @html_template_index = File.read(File.expand_path('../guestbook-data/index.template', __FILE__))
    @html_template_header = File.read(File.expand_path('../guestbook-data/dialog-header.template', __FILE__))
    @html_template_footer = File.read(File.expand_path('../guestbook-data/dialog-footer.template', __FILE__))

    dialog_list_html = ''
    dialogs.each do |dialog|
      safe_name = get_safe_name(dialog['print_name'])
      FileUtils.cp_r(File.expand_path(output_dir + '../../../media/' + safe_name, __FILE__), output_dir + '/media')
      title = safe_name
  ...
  def format_dialog(dialog, messages)
    ...
    if dialog['type'] != 'user'
      dialog_title = '%s' % CGI::escapeHTML(dialog['title'])
    ...
    media_dir = 'media/' + safe_name
    backup_file.puts(@html_template_header % [html_title, 'resources/' + safe_name + '-background.jpg', navigation, dialog_title, 'resources/'+ safe_name +'-avatar.jpg'])

    timestamps_every = $config['formatters']['guestbook']['timestamps_every'].to_i
    messages_per_page = $config['formatters']['guestbook']['paginate'].to_i
    ...
    messages.reverse_each do |msg|
      ...
      if msg['date']
        ...
        if $config['formatters']['guestbook']['use_utc_time']
      ...
      if msg['text']
        ...
        msg_body = "<div class=geo>Geo location: <a target='_blank' href='https://www.openstreetmap.org/?mlat=#{lat}&mlon=#{lon}#map=15/#{lat}/#{lon}'>(#{lat[0..8]},#{lon[0..8]})</a><br><iframe src='https://www.openstreetmap.org/export/embed.html?bbox=#{lon}%2C#{lat}%2C#{lon}%2C#{lat}&amp;layer=mapnik'></iframe></div>"
  ...
  def end_backup
    $log.info('HTML export finished, see: output/formatted/guestbook/index.html')
  end
